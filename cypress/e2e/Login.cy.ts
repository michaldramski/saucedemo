...
describe('Login tests', () => {
    ...
    it('Should not be able to login with unregistered user', () => {
        // Arrange
    ...
    it('Should be able to login with problem user', () => {
        // Act
        loginPage.login(Users.problem_user.login, Users.problem_user.password);
        // Assert
        cy.url().should('include', '/inventory.html');
    });

    it('Should be able to login with performance glitch user - one with performance issues - slow login', () => {
        // Act
        loginPage.login(
